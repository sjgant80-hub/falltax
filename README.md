# Falltax · sovereign return management + UK law engine

> Single-file return management + UK law research for 1-10 person HMRC-regulated tax adviser firms. One HTML file, runs entirely in your browser, no server, no telemetry, no signup. Client and return data lives in IndexedDB on the device you opened it on.

**Anchor of the law bundle** (prime 743). Sibling tools in the same bundle: `falltaxonboard` (CDD/AML/conflict register · 751), `falltaxpaper` (engagement + advice letters + precedents · 757), `falltaxpractice` (HMRC Accounts Rules + time + billing · 761). They sync over `BroadcastChannel('fall-law')` and signal estate-wide on `BroadcastChannel('fall-signal')`.

---

## For tax advisers

You're a 1-10 person firm. You want return management, conflict-checking, file retention, critical-date flagging, and a UK law research engine — without paying £80/user/month and without sending privileged data to someone's cloud.

**Open `index.html` in Chrome 113+ or Edge.** Nothing installs. Nothing phones home. First launch asks you for your firm name + HMRC number, then your first adviser (name, roll number, COLP/COFA toggles). That's it.

What you get:

- **Multi-return sidebar** with search by title / return ref / opposing party / client; filter by practice area, responsible tax adviser, status, risk, due date. Critical dates (limitation / hearing / next step) flag at 28d / 14d / 7d / today.
...
- **30 strategic weaves** — named patterns with archetype / move / authorities / opposition / counter / why-it-wins / example. Covers Limitation Act, Bolam/Bolitho, Montgomery, CFAs, Part 36, TUPE, Inheritance Act, Section 21, Equality Act 2010, Companies Act derivative + unfair prejudice, Norwich Pharmacal, Tax advisers Act assessment, and more. Apply a weave to a return to log strategic position.
- **UK Law corpus** — Tax advisers Act 1974, Tax Services Act 2007, Limitation Act 1980, CPR, Family Law Act 1996, Land Registration Act 2002, Wills Act 1837, Inheritance Act 1975, Equality Act 2010, ERA 1996, Companies Act 2006, Housing Act 1988, Children Act 1989, PACE 1984, MCA 1973.
- **Advice issuance** — write opinions in the return, sign with sha256 + adviser id + timestamp, retained 6 years per HMRC 13.5.
...
- **Q & A** — 14 one-tap T0 chips (PI limitation, CFA vs DBA, HMRC conduct, Bolam, conveyancing protocol, wasted costs, Inheritance Act, s.21 vs s.8, Equality Act, ET time limits, Norwich Pharmacal, CPR 31, without prejudice, privilege types). T3 cloud cascade (Anthropic → OpenAI → Gemini → OpenRouter) using your own API key — context includes the active return and client.
...
> **Disclaimer.** Falltax is a tool for HMRC-regulated UK tax advisers. It assists with return management, CDD, document generation, and HMRC Accounts Rules tracking. It is **not regulatory submission or tax opinion software**. The firm's COLP / COFA remain responsible. Sovereign — client data never leaves the device unless you export it.
...
{v:1, type:'return.created'|'return.updated'|'return.closed'|'return.reopened',
 source:'falltax', ts, payload:Return}
...
FALLTAX.WEAVES.length // 30
FALLTAX.T0_RULES.length // 14
FALLTAX.state.returns.length
FALLTAX.scanConflicts({clientName:'Patel'}) // exercise conflict scan
```
...
- **Discoverable** — indexed on the FallMarket catalog at [sjgant80-hub.github.io/fallmarket/listing.html?id=falltax](https://sjgant80-hub.github.io/fallmarket/listing.html?id=falltax)
...
- **Companion SDK** (`npm install @ai-native-solutions/falltax-sdk`) — programmatic JavaScript API for direct integration.
- **Companion MCP server** (`claude mcp add falltax -- npx -y @ai-native-solutions/falltax-mcp`) — Model Context Protocol server that any Claude/Cursor/Codex client can invoke as a tool.
- **Companion HTTP API** (`docker run --rm ghcr.io/sjgant80-hub/falltax-api`) — containerized REST endpoint for language-agnostic access.
...
- **Registry entry**: https://sjgant80-hub.github.io/fallmarket/listing.html?id=falltax
- **Live URL**: https://sjgant80-hub.github.io/falltax/
- **Source**: https://github.com/sjgant80-hub/falltax
